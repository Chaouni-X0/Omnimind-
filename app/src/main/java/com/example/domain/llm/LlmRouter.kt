...
class LlmRouter(
    ...
    suspend fun generate(prompt: String, minimumTier: Int = 1): Result<LlmReply> {
        val eligible = dao.getEnabledApiKeys().first()
        ...
        if (candidates.isEmpty()) return Result.failure(IllegalStateException("لا توجد نماذج AI مفعلة لهذا المستوى"))
        ...
        for (candidate in candidates) {
            ...
            runCatching { request(candidate, key, prompt) }
                .onSuccess {
                    dao.updateApiKey(candidate.copy(lastUsedTimestamp = System.currentTimeMillis(), lastError = null))
    ...
    private suspend fun requestGemini(key: String, model: String, prompt: String): String {
        val response = geminiApi.generateContent(
            model = model,
            apiKey = key,
            request = GeminiGenerateRequest(listOf(GeminiContent(listOf(GeminiPart(prompt)))))
        )
        return response.candidates?.firstOrNull()?.content?.parts?.firstOrNull()?.text.orEmpty()
    }

    private suspend fun requestOpenAiCompatible(config: ApiKeyConfig, key: String, model: String, prompt: String): String {
        val baseUrl = when (config.providerName.trim().lowercase()) {
            "openai" -> config.baseUrl ?: "https://api.openai.com/v1/"
            "openrouter" -> config.baseUrl ?: "https://openrouter.ai/api/v1/"
            "groq" -> config.baseUrl ?: "https://api.groq.com/openai/v1/"
    ...
    private suspend fun requestAnthropic(baseUrl: String?, key: String, model: String, prompt: String): String {
        ...
        val response = genericApi.post(
            ...
            mapOf("model" to model, "max_tokens" to 2048, "messages" to listOf(mapOf("role" to "user", "content" to prompt)))
        )
        check(response.isSuccessful) { "فشل المزود برمز HTTP ${response.code()}" }
        return response.body()?.getAsJsonArray("content")?.firstOrNull()?.asJsonObject?.get("text")?.asString.orEmpty()
    }

    private fun endpoint(baseUrl: String, path: String) = baseUrl.trimEnd('/') + "/" + path.trimStart('/')
    ...
    private fun safeError(error: Throwable): String = when (error) {
        ...
        is java.io.IOException -> "تعذر الاتصال بالمزود"
